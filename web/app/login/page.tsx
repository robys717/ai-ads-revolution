  ...
  async function handleSubmit(e: FormEvent) {
    ...
    if (error) {
      ...
      if (msg.includes("email") && msg.includes("confirm")) {
        setError(
          "Devi prima confermare la tua email. Controlla la posta e clicca sul link di conferma."
        );
      } else if (msg.includes("invalid login credentials")) {
        setError("Email o password non corrette.");
      } else {
        setError(error.message || "Errore di login. Riprova.");
      }
      return;
    }

    if (!data.session) {
      setError("Login non riuscito. Nessuna sessione attiva.");
      return;
    }
        ...
        <div className="flex items-center gap-3 mb-4">
          ...
          <div>
            ...
            <p className="text-xs text-slate-400">
              Login • Dashboard inserzionista
            </p>
          </div>
        </div>

        <h1 className="text-xl font-semibold mb-2">
          Login AI Ads Revolution
        </h1>
        <p className="text-sm text-slate-400 mb-5">
          Accedi alla tua{" "}
          <span className="font-medium">dashboard inserzionista</span>.
        </p>
