...
// Stripe (optional) – raw parser BEFORE JSON for webhook
let stripe = null;
...
// Click redirect + optional publisher share
...
app.post('/publisher/register', auth, (req,res)=>{
  ...
  const pub_key = 'pub_' + require('nanoid').nanoid(16);
  res.json({ pub_key, embed:`<script src=\"${process.env.BASE_URL}/ad-widget.js\" data-pub=\"${pub_key}\" data-location=\"Piacenza\" data-interests=\"pizza,ristorante\"></script>` });
});
...
app.get('/ads.json', (req,res)=>{
  ...
  db.all(\`SELECT c.*, (SELECT url FROM assets WHERE campaign_id = c.id ORDER BY id DESC LIMIT 1) as asset_url
          FROM campaigns c WHERE active=1 AND budget_cents>0\`, [], (err,rows)=>{
    if (err) return res.status(500).json({ error:'Failed feed' });
    const ranked = rows.map(c=>({ c, s: (function(c){ const userLoc=location.toLowerCase(); const userInts=interests.map(s=>s.toLowerCase()); const campLocs=(c.location||'').toLowerCase().split(',').map(s=>s.trim()).filter(Boolean); const campInts=(c.interests||'').toLowerCase().split(',').map(s=>s.trim()).filter(Boolean); let sc=0; if(userLoc && campLocs.some(l=>userLoc.includes(l)||l.includes(userLoc))) sc+=3; const ov=campInts.filter(ci=>userInts.includes(ci)).length; sc+=ov*2; sc+=Math.min(c.budget_cents/1000,5); const age=(Date.now()-new Date(c.created_at).getTime())/(1000*60*60*24); sc+=Math.max(0,3-(age/7)); return sc; })(c)}))
                      .sort((a,b)=>b.s-a.s).map(({c})=>c);
    const top = ranked[0];
    if(!top) return res.json({});
    res.json({
      id: top.id, title: top.title, description: top.description, asset_url: top.asset_url,
      click_url: `${process.env.BASE_URL}/click?campaign_id=${top.id}&pub=${encodeURIComponent(req.query.pub||'')}`
    });
  });
});

// Wallet + funding from wallet (Stripe optional for top-up)
